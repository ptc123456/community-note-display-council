...
- Second correction: accept explicit leader `SUCCESS`/`ERROR` receipt values while retaining conflict/unknown fail-closed behavior, and generate Explorer transaction links under `/transactions/`. Captured-receipt and route regressions were added; frontend verification is now `21/21` tests and build PASS. This presentation defect did not replay or reverse the successful create write.
- Retained deployment invocation failure: an attempted Vercel deployment was launched from the repository root rather than `frontend/` and failed at `npm ci` before build because no root package manifest exists. It never replaced the stable production alias. The corrected deployment was run from `frontend/`, built successfully, and reached `READY`; root `.vercel` state is ignored.
...
- Deadline clarity correction: the live UI now shows only a one-second `h/m/s` countdown, avoiding absolute-clock and time-zone ambiguity. One ticking clock drives countdown text, expiry labels, and action eligibility. Absolute case timestamps were also removed from the sidebar. Regression suite: `23/23`; production build: PASS.
- Corrected production deployment: exact frontend revision `efbb756`, Vercel deployment `dpl_2YTzQLcBmK6oCCbHuWBqJFCsTy7U`, `READY`, stable alias `https://community-note-display-council.vercel.app`. Chrome readback rendered `Submission: 0h 34m 38s remaining`, no absolute clock or UTC label, and remained disconnected after navigation. The invalid reputation control rendered its validation error with no stale score. These presentation corrections are closed.
- Vercel lock transaction: `0x84797360f3ace4783fd4a750f47cf4d9f5d4c73ec4b8e49d2add20b5f91e7655`, OKX actor `0x896E...e39E`, `lock_case(3)`, `FINALIZED / MAJORITY_AGREE`, 3 agree and 2 quorum-idle. Authoritative readback: case `3` changed `OPEN -> LOCKED`, `locked_at=1787488841`, note count remained `1`.
- Vercel evaluate transaction: `0x01bb42510b84a93a65adfb8718f3ac1a84a2bab71c405549714c592a8d3730cc`, same exact OKX provider/actor, `evaluate_case(3)`, `FINALIZED / MAJORITY_AGREE`, 3 agree and 2 quorum-idle. Readback: `CHALLENGE`, selected note `0`, `DISPLAY`, score `9735`, rationale digest `27e8e8614bef5bb4dda964c862945d405c1711ac29f2899157d19fa1198636b8`, challenge deadline `1787492521`.
- Vercel challenge transaction: `0xeb7d038524e9ef9e8fa6ec9bee0bf6852cd7f1e46832521544817a051eafc4b6`, same actor, `submit_challenge(3, reason, [https://example.com/])`, `FINALIZED / MAJORITY_AGREE`, 3 agree and 2 quorum-idle. Authoritative `get_challenge_count(3)=1`; challenge `0` exactly matched actor/reason/source; case remained `CHALLENGE`. The UI disabled repeat challenge for the same connected actor.
- Challenge boundary control: no `resolve_challenges(3)` write was sent before `1787492521`; the post-deadline write below is the only Vercel resolution attempt for case `3`.
- Vercel resolve transaction: `0x84b734ee89a5b1c8a7d95c50e1aeb86e8f21cfcc4265bfaae51c3ff041656edc`, OKX actor `0x896E...e39E`, submitted only after challenge deadline, `FINALIZED / MAJORITY_AGREE`, 3 agree and 2 quorum-idle. Authoritative readback: case `3` changed `CHALLENGE -> EVALUATED`; final note `0`; consequence `DISPLAY`; score `10000`; rationale digest `39658dd39b387749f00af1baafd73861ea99d4c690cb6f4ea501820906dc6a39`; `impactful_challenge_ids=[]`; `resolved_at=1787492621`.
